...
export async function POST(req: Request) {
  ...
  if (!custom.acceptedInputs.includes('video' as 'text')) {
    return jsonErrorCors(400, `Model "${modelId}" does not accept video prompts`);
  }
  ...
  try {
    ...
    const response = await videosGenerations({
  ...
  if (fallback && fallback.type === 'video') {
    try {
      return await videosGenerations({
